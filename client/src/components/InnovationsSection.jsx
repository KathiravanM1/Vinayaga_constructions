      ...
      <div className="container">
        ...
        <div className="grid gap-px bg-foreground/5 sm:grid-cols-2 lg:grid-cols-4">
          {(services.length > 0 ? services : []).map((s, i) => (
            <div key={s._id} className="flex items-center gap-4 bg-secondary px-6 py-5 transition-colors hover:bg-secondary/80">
        ...
        <div className="mt-16 flex flex-wrap items-center justify-center gap-4">
          {badges.map((b) => (
            <span key={b} className="border copper-border px-6 py-2.5 text-xs font-medium uppercase tracking-[0.15em] copper-text">
              ✓ {b}
            </span>
          ))}
        </div>
